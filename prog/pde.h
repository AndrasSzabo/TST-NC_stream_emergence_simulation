...
class PDE {
  ...
  */
  ...
  /*! \brief Plots the magnitude of the local gradient from one PDE layer
    \param l: The PDE plane to be used.
    \param k: The gradient magnitude 'm' is plotted with a colour = 100*m/(m+k)+155, such that parameter 'k' defines the gradient magnitude value assigned to colour 205 halfway through the scale (from 155 to 255). 
  */
  void PlotGradientMagnitude(Graphics *g, const int l=0, const int k=100);
  void SaveGradientMagnitude(const int l=0, const char * fname="ccgrad.txt");
  void SaveConcentration(const int l=0, const char * fname="cc.txt");
...
//      }
  ...
  */
  ...
  void AbsorbingBoundariesExt(void);

  /*! \brief Implementation of continuing gradient boundaries.
    
  Called internally (optionally) by Diffuse(). */
  void GradientBoundaries(void);
  ...
  */
  void Secrete(CellularPotts *cpm, int ipmcs);
  ...
  void ClearMatrix(CellularPotts *cpm, int layer);
  ...
  */
  ...
  void IteratePDE(CellularPotts *cpm);
