...
{ 
  ...
  printf("store: %s, graphics: %s\n",par.store ? "true" : "false", par.graphics ? "true" : "false");
  printf("depth/8 = %d\n",depth/8);
  if (pseudoCol8) {
...
void X11Graphics::Write(char *fname, int quality) {
  ...
  // Value of quality is -1, so this output will not be called normally:
  if (quality == 0){
    cerr << "Writing a PNG picture (" << quality << ")\n";
  }
