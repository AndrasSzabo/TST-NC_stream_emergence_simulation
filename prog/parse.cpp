...
double *dgetparlist(FILE *fp,const char *parameter, int n, const char *def_token, bool wrapflag) 
{
...
char *SearchToken(FILE *fp, const char *token,bool wrapflag) 
{
  ...
  if (ferror(fp)) /* error occured */
    {
      if (tokenplusspace) free(tokenplusspace);
...
// The below 'free' is not needed, do it at the end:
//    if (line) free(line);
  }
  if (line) free(line);
  if (tokenplusspace) free(tokenplusspace);
