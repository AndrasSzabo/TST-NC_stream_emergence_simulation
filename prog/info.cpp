...
void Info::Menu() {
  ...
  case 'K':
    {
      extern Parameter par; 
      *os << "lambda2 = " << par.lambda2 << endl;
      *os << "new? = "; ReadDouble(stdin,&par.lambda2);  
      *os << "new lambda2 = " << par.lambda2 << endl;
    }
    break;
  ...
  case 'k':
    {
      extern Parameter par; 
      *os << "target_length = " << par.target_length << endl;
      *os << "new? = " ; ReadNumber(stdin,&par.target_length);  
      dish->CPM->ResetTargetLengths();
      *os << "new target_length = " << par.target_length << endl;
    }
    break;
  ...
  case 'J':
    {
      extern Parameter par; 
      *os << "J[1][0] = " << par.J[1][0] << endl;
      *os << "new? = "; ReadDouble(stdin,&par.J[1][0]);  
      par.J[0][1]=par.J[1][0];
  ...
  case 'j':
    {
      extern Parameter par;
      *os << "J[1][1] = " << par.J[1][1] << endl;
      *os << "new? = " ; ReadDouble(stdin,&par.J[1][1]);
      vector<Cell>::const_iterator i;
      for ( (i=dish->cell.begin(), i++); i!=dish->cell.end(); i++) {
        i->ReadStaticJTable();
      }
      *os << "new J[1][1] = " << par.J[1][1] << endl;
    }
    break;
  ...
  case 'P':
    {
      extern Parameter par; 
      *os << "Polarisation = " << par.Polarisation << endl;
      *os << "new ? = "; ReadDouble(stdin,&par.Polarisation);  
      *os << "new Polarisation = " << par.Polarisation << endl;
    }
    break;
  ...
  case 'D':
    {
      extern Parameter par; 
      *os << "Pol_decay = " << par.Pol_decay << endl;
      *os << "new? = "; ReadDouble(stdin,&par.Pol_decay);  
      *os << "new Pol_decay = " << par.Pol_decay << endl;
    }
    break;
  ...
  case 'p':
	...
	while (graphics->GetXYCoo(&x,&y)!=1);
	dumpcell=&dish->cell[dish->CPM->Sigma(x/2,y/2)];
	double px=0, py=0; dumpcell->getPolarisation(&px, &py);
	printf("Polarisation of cell %d is %f  %f.\n",dumpcell->Sigma(), px, py);
		printf("----------------------------------------------\n\n");
      } while (dumpcell->Sigma()!=0);
    }
    break;
